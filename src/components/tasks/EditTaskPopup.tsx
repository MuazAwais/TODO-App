// EditTaskPopup Component - Edit existing task in a popup
// Uses Formik + Yup for form handling and validation
// Similar to TaskAdderPopup but pre-fills with existing task data
...
interface EditTaskPopupProps {
  task: Task;
  // Optional callback when task is updated successfully
  onTaskUpdated?: () => void;
...
export default function EditTaskPopup({
  ...
  // Convert task data to form values
  // Handle Unix timestamp conversion for dueDate
  const getInitialValues = (): TaskFormValues => {
    let dueDateString = "";
    if (task.dueDate) {
      // Convert Unix timestamp (milliseconds) to datetime-local format
      const date = new Date(
        typeof task.dueDate === "number" ? task.dueDate : parseInt(task.dueDate)
      );
      ...
      dueDateString = `${year}-${month}-${day}T${hours}:${minutes}`;
    }
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      // Update task via RTK Query mutation
